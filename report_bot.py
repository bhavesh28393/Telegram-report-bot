...
def cmd_login(message):
    ...
    if len(sessions) >= MAX_ACCOUNTS_PER_USER:
        bot.reply_to(message, f"❌ Max {MAX_ACCOUNTS_PER_USER} accounts reached!")
        return
...
def cmd_pair(message):
    ...
    if not sessions:
        bot.reply_to(message, "❌ No accounts! Use /login first.")
        return
    
    if len(sessions) == 1:
        set_paired_accounts(user_id, list(sessions.keys()))
        bot.reply_to(message, f"✅ Auto-paired: {list(sessions.keys())[0]}\nUse /report")
    else:
...
def cmd_report(message):
    ...
    bot.reply_to(message, "🎯 Send Instagram username or URL:")
...
def cmd_status(message):
    ...
    if active:
        bot.reply_to(message, f"⚔️ Active reports: {len(active)}")
    else:
...
def cmd_stop(message):
    ...
    for rid in list(active_reports.keys()):
        if rid.startswith(str(user_id)) and active_reports[rid]:
            ...
            return
    bot.reply_to(message, "❌ No active report")
...
def cmd_logout(message):
    ...
    bot.reply_to(message, f"Accounts:\n{session_list}\n\nSend nickname to remove:")
...
def handle_conversation(message):
    ...
    if step == "username":
        ...
        if result["status"] == "success":
            ...
            bot.reply_to(message, "❌ Verification failed! Use /login again")
            del user_states[user_id]
            del user_logins[user_id]
        ...
        bot.reply_to(message, f"📊 Report limit (1-{MAX_REPORT_LIMIT}):")
        ...
        try:
            ...
            if 1 <= report_limit <= MAX_REPORT_LIMIT:
                ...
                reasons = "\n".join([f"{k}. {REPORT_REASONS[k]['name']}" for k in REPORT_REASONS])
                bot.reply_to(message, f"Select reason:\n{reasons}\n\nSend number (1-6):")
            else:

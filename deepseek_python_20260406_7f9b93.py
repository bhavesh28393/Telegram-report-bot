...
def take_profile_screenshot(session_data, username, user_id):
    """Take screenshot of target profile for confirmation"""
    try:
        with sync_playwright() as p:
            ...
            with open(filename, 'rb') as photo:
                bot.send_photo(user_id, photo, caption=f"📸 Is this the correct profile?\n\nUsername: @{username}")
            
            os.remove(filename)
            browser.close()
            return True
    except Exception as e:
        bot.send_message(user_id, f"⚠️ Could not fetch screenshot: {str(e)[:50]}")
        return False

# ============= LOGIN FUNCTION =============
def instagram_login_smart(user_id, username, password, verification_method=None, verification_code=None):
    try:
        ...
        with sync_playwright() as p:
            ...
            time.sleep(2)
            take_screenshot(page, "01_instagram_home", user_id)
            
            try:
                accept_btn = page.wait_for_selector('button:has-text("Accept")', timeout=10000)
                if accept_btn:
                    accept_btn.click()
                    time.sleep(1)
                    take_screenshot(page, "02_cookies_accepted", user_id)
            except:
                pass
            
            bot.send_message(user_id, f"✏️ Entering username: {username}")
            username_input = page.wait_for_selector('input[name="username"]', timeout=30000)
            username_input.fill(username)
            time.sleep(1)
            take_screenshot(page, "03_username_entered", user_id)
            
            bot.send_message(user_id, "➡️ Moving to password field...")
            page.keyboard.press("Tab")
            time.sleep(1)
            take_screenshot(page, "04_tab_pressed", user_id)
            
            bot.send_message(user_id, "🔒 Entering password...")
            password_input = page.query_selector('input[name="password"]')
            password_input.fill(password)
            time.sleep(1)
            take_screenshot(page, "05_password_entered", user_id)
            
            bot.send_message(user_id, "🔐 Pressing Enter to login...")
            page.keyboard.press("Enter")
            time.sleep(5)
            take_screenshot(page, "06_after_enter", user_id)
            
            page.wait_for_load_state("networkidle", timeout=30000)
            time.sleep(2)
            
            current_url = page.url
            bot.send_message(user_id, f"📍 Current page: {current_url[:60]}...")
            
            if "accounts/login" not in current_url and "challenge" not in current_url and "two-factor" not in current_url:
                storage_state = context.storage_state()
                browser.close()
                take_screenshot(page, "07_login_success", user_id)
            ...
            if "challenge" in current_url or "two-factor" in current_url:
                take_screenshot(page, "07_verification_needed", user_id)
                bot.send_message(user_id, "🔐 Verification required!")
                
                page_content = page.content()
                if "approve" in page_content.lower() or "device" in page_content.lower():
                    bot.send_message(user_id, "📱 Device approval request detected!\n\nPlease check your Instagram app and approve the login.\n\nWaiting 45 seconds...")
                    take_screenshot(page, "08_device_approval", user_id)
                    
                    for i in range(45):
                        time.sleep(1)
                        if i % 15 == 0 and i > 0:
                            bot.send_message(user_id, f"⏳ Waiting... {45-i} seconds remaining")
                    
                    page.reload()
                    time.sleep(3)
                    take_screenshot(page, "09_after_reload", user_id)
                    
                    if "accounts/login" not in page.url and "challenge" not in page.url:
                        storage_state = context.storage_state()
                        browser.close()
                        bot.send_message(user_id, "✅ Device approved!")
                        return {"status": "success", "storage_state": storage_state}
                ...
                if verification_method and verification_code:
                    ...
                    try:
                        ...
                        time.sleep(1)
                        take_screenshot(page, "10_code_entered", user_id)
                        
                        page.keyboard.press("Enter")
                        time.sleep(5)
                        take_screenshot(page, "11_after_code_submit", user_id)
                        ...
                        browser.close()
                        return {"status": "failed", "error": "Could not enter code"}
                else:
                    browser.close()
                    return {"status": "verification_needed", "options": ["sms", "email"]}
            
            browser.close()
            return {"status": "failed", "error": "Login failed - check username/password"}
...
def handle_conversation(message):
    ...
    # Report flow - target
        ...
        # Take screenshot of the target profile for confirmation
        bot.send_message(user_id, "📸 Fetching profile screenshot for confirmation...")
        
        # Get first paired account to take screenshot
        paired = get_paired_accounts(user_id)
        sessions = get_user_sessions(user_id)
        ...
        if first_account and first_account in sessions:
            ...
            bot.send_message(user_id, f"⚠️ Could not fetch screenshot. Proceeding with username: @{username}")
...
if __name__ == "__main__":
    ...
    print("="*60)
    print("✅ Bot running...")
    print("✅ Sequential Login: Username → Tab → Password → Enter")
    print("✅ Profile Screenshot Confirmation Active")
    print("="*60)
